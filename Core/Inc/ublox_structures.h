...
PACK(
    struct NavSVInfo{
        ...
        uint16_t reserved2;
        struct SVInfoReapBlock svinfo_reap[MAXCHAN]; // NOTE: TODO: True max needs to be confirmed
        uint8_t checksum[2];
});
...
PACK(
    struct NavDGPS{
        ...
        struct NavDGPSReap nav_dgps_reap;  // repeated portion of NAV-DGPS message
        uint8_t checksum[2];
});
...
PACK(
    struct RawMeas{
        ...
        uint8_t reserved;
        struct RawMeasReap rawmeasreap[MAX_SAT];
        uint8_t checksum[2];
});
...
PACK(
    struct SVStatus{
        ...
        uint8_t numSV;      // # of SVs following
        struct SVStatusReap svstatusreap[100]; // NOTE: TODO: Find the max repititions possible for this!! max thus far: (71)
        uint8_t checksum[2];
});
